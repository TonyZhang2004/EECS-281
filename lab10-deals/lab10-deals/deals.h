...
cost best_price(const std::vector<cost>& prices) {
    ...
    vector<vector<cost>> dp(6, vector<cost>(prices.size(), numeric_limits<cost>::max() / 4));
    // initialize the first 5 column
    dp[0][0] = discounted(prices[0]);
    dp[1][0] = prices[0];
    for (uint32_t i = 1; i < 5; i++) {
        dp[i + 1][i] = dp[i][i - 1] + prices[i];
        dp[0][i] = dp[0][i - 1] + discounted(prices[i]);
        for (uint32_t j = 1; j < 6; j++) {
            dp[j][i] = min(dp[j][i - 1] + discounted(prices[i]), dp[j - 1][i - 1] + prices[i]);
        }
    }
    for (uint32_t i = 5; i < prices.size(); i++) {
        dp[0][i] = min(dp[0][i - 1] + discounted(prices[i]), dp[5][i - 1]);
        for (uint32_t j = 1; j < 6; j++) {
            dp[j][i] = min(dp[j][i - 1] + discounted(prices[i]), dp[j - 1][i - 1] + prices[i]);
        }
    }

    cost ans = dp[0][prices.size() - 1];
    for (uint32_t i = 1; i < 6; i++) {
        ans = min(ans, dp[i][prices.size() - 1]);
    }
