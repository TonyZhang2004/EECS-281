...
void Battle::push_med_heap(Zombie* zb) {
	...
	else if (this->max_med_heap.size() > this->min_med_heap.size()) {
...
void Battle::print_stats() {
	...
	size_t num_limit = min(this->option.N, this->master_list.size());
	cout << "Most active zombies:\n";
	for (size_t i = 0; i < num_limit; i++) {
		cout << max_stats_heap.top()->get_name() << " " << max_stats_heap.top()->get_act_rounds() << "\n";
		max_stats_heap.pop();
	}

	cout << "Least active zombies:\n";
	for (size_t i = 0; i < num_limit; i++) {
		cout << min_stats_heap.top()->get_name() << " " << min_stats_heap.top()->get_act_rounds() << "\n";
		min_stats_heap.pop();
	}
}
...
void Battle::player_shoot_zombies() {
	while (this->quiver != 0 && !pq.empty()) {
		...
		pq.top()->being_shot(this->quiver);
		if (pq.top()->get_health() == 0) { // The current zombie is dead
			if (this->option.verbose) {
				cout << "Destroyed: ";
				pq.top()->display();
			}
...
void Battle::push_stats_heap(Zombie* const zb) {
