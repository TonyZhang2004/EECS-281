...
String& String::erase(size_t pos, size_t len) {
    ...
    for (size_t i = 0; i < sz - idx; i++) {
        cstr[pos + i] = cpy.cstr[idx + i];
    }
    sz -= len;
    cstr[sz] = a_null_byte;
    return *this;
...
String& String::insert(size_t pos, const String& str) {
    ...
    for (size_t i = 0; i < sz - pos; i++) {
        cstr[str.sz + sz - 1 - i] = cstr[sz - 1 - i];
    }
    /*for (size_t i = sz - 1; i >= int(pos); i--) {
        cstr[i + str.sz] = cstr[i];
    }*/
    for (size_t i = pos; i < str.sz + pos; i++) {
        cstr[i] = cpy.cstr[i - pos];
    }
    cstr[sz + str.sz] = a_null_byte;
    sz += str.sz;
    return *this;
}  // insert()
